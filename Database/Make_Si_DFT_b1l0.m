...
for i = 1:length(E0)
    ...
    if energy > 2*Si_DFT_b1l0.Eg + Si_DFT_b1l0.Evb
        osc.eloss = eps:(energy-eps)/(N-1):energy;
        Si_DFT_b1l0.DIIMFP(:,1,i) = osc.eloss;
        [iimfp, Si_DFT_b1l0.DIIMFP(:,2,i)] = ndiimfp(osc,energy,elf,q,omega);
        eloss_interp = Si_DFT_b1l0.Eg:(energy-2*Si_DFT_b1l0.Eg-Si_DFT_b1l0.Evb)/N:energy-Si_DFT_b1l0.Eg-Si_DFT_b1l0.Evb;
        iimfp_interp = interp1(osc.eloss,iimfp,eloss_interp);
        Si_DFT_b1l0.l_in(i) = 1/trapz(eloss_interp/h2ev,iimfp_interp)*a0;
    else
