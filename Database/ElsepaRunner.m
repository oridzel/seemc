classdef ElsepaRunner
    ...
    methods (Static)
        function [Res] = RunElsepa(Composition, E0)
            ...
            sigma_tr2 = zeros(length(Composition.Z),numel(E0));
            ...
            for z = 1:length(Composition.Z)
                ...
                for i=1:numel(E0)
                    ...
                    a = El.textdata(~cellfun('isempty',regexp(El.textdata,'2nd transport cross section')));
                    sigma_tr2(z,i) = str2double(extractBetween(a{1},"cm**2 = "," a0**2"))*a0^2;
                    ...
                    if z == length(Composition.Z)
                        data.x = El.data(:,1)/180*pi;
                        data.y(:,i) = sum(decs_all(:,:,i).*Composition.index')/sumweights;
                        data.sigma_el = sum(sigma_el.*Composition.index')/sumweights;
                        data.sigma_tr1 = sum(sigma_tr1.*Composition.index')/sumweights;
                        data.sigma_tr2 = sum(sigma_tr2.*Composition.index')/sumweights;
                        data.decs_all = decs_all;
                        data.sigma_el_all = sigma_el;
                        data.sigma_tr1_all = sigma_tr1;
                        data.sigma_tr2_all = sigma_tr2;
                        res(i) = data;
                    end                                       
                end
            end
