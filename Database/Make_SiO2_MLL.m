...
for i = 1:numel(E0)
    SiO2_MLL.Elastic.sigma_el(i) = data(i).sigma_el;
    SiO2_MLL.Elastic.l_el(i) = 1/(data(i).sigma_el*a0^2*SiO2_MLL.Density);
    SiO2_MLL.Elastic.l_tr(i) = 1/(data(i).sigma_tr1*a0^2*SiO2_MLL.Density);
    SiO2_MLL.DECS.y(:,i) = data(i).y;
end
%}
...
for i = 1:length(E0)
    if E0(i) > 2*SiO2_MLL.Eg + SiO2_MLL.Evb
        ...
        eloss = SiO2_MLL.Eg:(energy-SiO2_MLL.Eg)/(N-1):energy;
        if energy < 10
            osc.eloss = SiO2_MLL.Eg:0.01:energy;
        else
            osc.eloss = SiO2_MLL.Eg:0.2:energy;
        end
        SiO2_MLL.DIIMFP(:,1,i) = eloss;
        [iimfp, diimfp] = ndiimfp(osc,E0(i));
        SiO2_MLL.DIIMFP(:,2,i) = interp1(osc.eloss,diimfp,eloss);
