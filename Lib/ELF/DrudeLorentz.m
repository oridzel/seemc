function [eps_re, eps_im]=DrudeLorentz(q,w_global,omega0,gamma,alpha_g,beta_g,alpha,FermiEnergy)
    ...
    divisor = mm.^2 + w_global.^alpha_g*gamma^(4-alpha_g);

    eps_re = bsxfun(@minus,w_global.^2,w_at_q.^2)*(omega0.^2)./divisor;
    eps_im = gamma*(omega0.^2).*bsxfun(@rdivide,w_global.^beta_g,divisor);
